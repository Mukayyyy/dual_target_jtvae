    ...
    def train(self):
        ...
        for g in range(self.config.get('num_generations')):
            ...
            if g == 0:
                ...
                self.dataset.change_data(self.new_pop, g)
            ...
            properties = samples.loc[:,['SAS', 'logP', 'CA9', 'GPX4']] # SAS: the smaller the better, logP: the smaller the better, score: the smaller the better
            ...
            if g<self.config.get('num_generations'):
                ...
                if self.population_size>0:
                    ...
                    # ind = np.random.choice(10000, 2000, replace=False )
            ...
            # with open(self.config.get('BCC_path'), 'r') as f:
                # BCC_data = f.readlines()
            # BCC_data = self.get_properties(BCC_data)
            # adding molecules from BCC to the first pop (plus last few mols from zinc dataset)
            if g == 0:
                combined_samples = self.dataset.get_smile_data().iloc[-20500:]
...
#            print('length={}'.format(length))
    ...
    def get_z(self, data=None):
        ...
        for batch in loader:
            ...
            with torch.no_grad():
                loss, kl_div, wacc, tacc, sacc, z, mu, logvar, predicted = self.trainer.model(batch, self.config.get('a_beta'))
