...
from fast_jtnn import JTNNEncoder, MPN, JTMPN, Vocab, MolTree
...
def tensorizeTrainData(smiles, assm=True):
...
class MolTreeDataset(Dataset):
    ...
    # Process JTVAE training data and set up configs
    def change_data(self, data, g):
        self.data = data
        ...
        new_train = self.data['smiles'].tolist()

        with open(r'/pops/zbd-pop-%d' % g, 'w') as fp:
            for item in new_train:
                fp.write('%s\n' % item)

        smiles_file = 'zbd-pop-' + g

        self.config.set('processed_smile', 'learner/pops/' + smiles_file)
       
        pool = Pool(18)
        num_splits = 50
        all_data = pool.map(tensorizeTrainData, new_train)
        ...
        folder_name = 'new-pop-' + g

        subprocess.Popen("mkdir" + folder_name, shell=True, start_new_session=True)
        subprocess.Popen("mv" + 'tensor*' + folder_name , shell=True, start_new_session=True)

        self.confid.set('train_path', 'learner/' + folder_name)
