...
def tree_decomp(mol):
    ...
    ssr = [list(x) for x in Chem.GetSymmSSSR(mol)]  # get the list of simple rings
    cliques.extend(ssr)
    ...
    for i in range(len(cliques)):
        ...
        for atom in cliques[i]:
            for j in nei_list[atom]:
                if i >= j or len(cliques[j]) <= 2: continue   # regular bond, not ring
                inter = set(cliques[i]) & set(cliques[j])
                if len(inter) > 2:
                    cliques[i].extend(cliques[j])    # merge two rings
                    cliques[i] = list(set(cliques[i]))       # drop duplicates
                    cliques[j] = []
